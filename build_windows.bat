...
REM [0/4] pythoncore(Scripts 미등록) 대응: Scripts 폴더를 PATH 에 임시 추가
...
REM  누락 방지를 위해 --add-data 로 명시. tkinter 저장 대화상자 대비 --collect-all tkinter.
REM  streamlit 메타데이터/정적파일 누락 대비 --copy-metadata / --collect-all streamlit 추가.
